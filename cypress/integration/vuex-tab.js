...
suite("vuex tab", () => {
  ...
  it("should filter history", () => {
    cy.get(".left .search input")
      .clear()
      .type("inc");
    cy.get('.history .entry[data-active="true"]').should("have.length", 2);
    cy.get('.history .entry[data-active="true"].inspected').should(
      "have.length",
      0
    );
    cy.get('.history .entry[data-active="true"].active').should(
      "have.length",
      0
    );
  ...
  it("should inspect state", () => {
    cy.get(".history .entry .mutation-type")
      .eq(2)
      .click();
    cy.get(".history .entry")
      .eq(2)
    ...
    cy.get(".data-field .key")
      .contains("lastCountPayload")
      .click();
    cy.get(".vuex-state-inspector").then(el => {
  ...
  it("should time-travel", () => {
    cy.get(
      '.history .entry[data-index="4"] .entry-actions .action-time-travel'
    ).click({ force: true });
    ...
    cy.get('.history .entry[data-index="3"] .mutation-type').click({
      force: true
    });
    ...
    cy.get(
      '.history .entry[data-index="3"] .entry-actions .action-time-travel'
    ).click({ force: true });
    cy.get('.history .entry[data-index="3"]')
      .should("have.class", "inspected")
      .should("have.class", "active");
    ...
    // Base state
    cy.get('.history .entry[data-index="0"] .mutation-type').click({
      force: true
    });
    ...
    cy.get(
      '.history .entry[data-index="2"] .entry-actions .action-time-travel'
    ).click({ force: true });
  ...
  it("should revert", () => {
    cy.get('.history .entry[data-index="5"] .mutation-type').click({
      force: true
    });
    cy.get('.history .entry[data-index="5"]')
      .find(".action-revert")
      .click({ force: true });
    cy.get('.history .entry[data-active="true"]').should("have.length", 5);
    cy.get('.history .entry[data-index="4"]')
  ...
  it("should commit", () => {
    cy.get('.history .entry[data-index="4"] .action-commit').click({
      force: true
    });
    cy.get('.history .entry[data-active="true"]').should("have.length", 1);
    cy.get('.history .entry[data-index="0"]')
  ...
  it("should import vuex state", () => {
    ...
    cy.get(".import-state textarea")
      .clear()
      .type(
        '{{}"count":42,"date":"[native Date Fri Dec 22 2017 10:12:04 GMT+0100 (CET)]","nested":{{}"foo":"meow"},"instant":{{}"hey":"hi"}}'
      );
    cy.wait(500);
    ...
    cy.get(".vuex-state-inspector").then(el => {
      ...
      expect(el.text()).to.include(
        "date:" + new Date("Fri Dec 22 2017 10:12:04 GMT+0100 (CET)")
      );
    });
    cy.get(".import").click();
