<template>
  ...
  >
    <div class="header" @click="selectDefault">
      <slot name="header" />
    </div>

    <div v-show="isOpen" class="group-dropdown-options">
      <template v-for="option of options">
        <slot :ref="option.name" name="option" :option="option" />
      </template>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    selectDefault() {
      // Select next
      let i = this.options.findIndex(o => o.name === this.value);
      if (i === -1) {
        i = 0;
      } else {
        i++;
