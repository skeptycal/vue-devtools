import { UNDEFINED, SPECIAL_TOKENS, parse } from "src/util";
...
export default {
  ...
  computed: {
    ...
    isSubfieldsEditable() {
      return (
        this.isEditable &&
        (this.valueType === "array" || this.valueType === "plain-object")
      );
    },
    ...
    duplicateKey() {
      return (
        this.parentField &&
        this.parentField.value.hasOwnProperty(this.editedKey)
      );
    },
    ...
    keyValid() {
      return (
        this.editedKey &&
        (this.editedKey === this.field.key || !this.duplicateKey)
      );
    },
  ...
  methods: {
    openEdit(focusKey = false) {
      if (this.isValueEditable) {
        ...
        this.editedValue = this.transformSpecialTokens(
          JSON.stringify(this.field.value),
          true
        );
        ...
        this.$nextTick(() => {
          const el = this.$refs[
            focusKey && this.renamable ? "keyInput" : "editInput"
          ];
          el.focus();
    ...
    submitEdit() {
      if (this.editValid) {
        ...
        const newKey =
          this.editedKey !== this.field.key ? this.editedKey : undefined;
    ...
    containsEdition() {
      return (
        currentEditedField && currentEditedField.path.indexOf(this.path) === 0
      );
    },
