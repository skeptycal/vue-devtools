...
export default {
  ...
  methods: {
    ...
    getMarkerStyle(marker) {
      const start =
        Math.round(this.currentBenchmark.start / FPS_MARKERS_PRECISION) *
    ...
    onMetricClick(metric) {
      const index =
        Math.round(metric.time / FPS_MARKERS_PRECISION) * FPS_MARKERS_PRECISION;
...
.bar-wrapper {
  ...
  &:hover {
    background: rgba($vue-ui-color-primary, 0.1);
