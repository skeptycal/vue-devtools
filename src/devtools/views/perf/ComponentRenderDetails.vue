<template>
  <ScrollPane class="component-render-details">
    <div v-if="!entry" class="vue-ui-empty">
    ...
    <template v-else>
      ...
      >
        <div class="header">
          <div v-for="column of columns" :key="column" class="column">
            {{ column }}
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    componentName() {
      return (
        (this.$shared.classifyComponents
          ? classify(this.entry.id)
          : this.entry.id) || "Anonymous Component"
      );
    },
