...
const mutations = {
  FLUSH(state, payload) {
    ...
    state.inspectedInstanceId = state.inspectedInstance
      ? state.inspectedInstance.id
      : null;
    ...
    if (process.env.NODE_ENV !== "production") {
      Vue.nextTick(() => {
        console.log(
          `devtools render took ${window.performance.now() - start}ms.`
        );
        if (inspectTime != null) {
          console.log(
            `inspect component took ${window.performance.now() -
              inspectTime}ms.`
          );
          inspectTime = null;
        }
...
const actions = {
  toggleInstance(
    ...
    { instance, expanded, recursive, parent = false } = {}
  ) {
