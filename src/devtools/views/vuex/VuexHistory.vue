    ...
    <action-header slot="header">
      ...
      <a
        v-tooltip="
          $t(
            `VuexHistory.${
              $shared.recordVuex ? 'stopRecording' : 'startRecording'
            }.tooltip`
          )
        "
    ...
    >
      <template slot-scope="{ item: entry, index, active }">
        ...
        >
          ...
          <span v-if="inspectedIndex === -1" class="label inspected"
            >inspected</span
          >
        </div>
        <div
        ...
        >
          ...
          <span v-if="isInspected(index, entry)" class="label inspected"
            >inspected</span
          >
        </div>
      </template>
  ...
  computed: {
    ...
    filter: {
      ...
      set(filter) {
        ...
        this.$store.dispatch(
          "vuex/inspect",
          filter ? -1 : this.history.length - 1
        );
      }
    },
    ...
    highDensity() {
      ...
      return (
        (pref === "auto" && this.filteredHistory.length > 7) || pref === "high"
      );
    }
  },
