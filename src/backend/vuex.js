...
class VuexBackend {
  ...
  addModule(path, module, options) {
      ...
      const replaceNestedStates = nestedModule => {
        if (nestedModule.modules) {
          Object.keys(nestedModule.modules).forEach(key => {
            ...
            if (child.state) {
              state =
                typeof child.state === "function" ? child.state() : child.state;
            }
    ...
    const key = path.join("/");
    const moduleInfo = (this.registeredModules[key] = this.allTimeModules[
      key
    ] = {
      path,
      module: fakeModule,
  ...
  replayMutations(index) {
    if (!SharedData.vuexNewBackend) {
      const snapshot =
        index === -1 ? this.legacyBaseSnapshot : this.mutations[index].snapshot;
    ...
    if (stateSnapshot.index === index) {
      ...
      for (let i = stateSnapshot.index + 1; i <= index; i++) {
        ...
        if (mutation.registerModule) {
          ...
          mutation.unregisterModule &&
          this.hasModule(mutation.payload.path)
        ) {
          ...
          try {
            ...
            if (Array.isArray(mutation.handlers)) {
              if (this.isLegacy) {
                mutation.handlers.forEach(handler =>
                  handler(this.store.state, ...payload)
                );
              } else {
      ...
      if (!isProd)
        console.log(`replayed ${index - stateSnapshot.index} mutation(s)`);
    }
  ...
  cacheStateSnapshot(index, permanent = false) {
    ...
    if (
      this.stateSnapshotCache.filter(s => !s.permanent).length >
      SharedData.cacheVuexSnapshotsLimit
    ) {
      const i = this.stateSnapshotCache.findIndex(s => !s.permanent);
      if (i !== -1) {
        if (!isProd)
          console.log(
            "clean cached snapshot",
            this.stateSnapshotCache[i].index
          );
