...
function replacer(key) {
  ...
  if (Array.isArray(val)) {
    ...
    if (proto === "[object Map]") {
      ...
      return encodeCache.cache(val, () =>
        getCustomComponentDefinitionDetails(val)
      );
    }
...
export function getCustomRefDetails(instance, key, ref) {
  ...
  if (Array.isArray(ref)) {
    value = ref
      .map(r => getCustomRefDetails(instance, key, r))
      .map(data => data.value);
  } else {
    let name;
    if (ref._isVue) {
    ...
    value = {
      _custom: {
        display:
          ...
          (ref.className
            ? ` <span class="attr-title">class</span>="${ref.className}"`
            : "") +
          "&gt;",
